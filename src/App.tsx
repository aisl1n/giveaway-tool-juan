      ...
      {isDrawModalOpen ? (
        <div className="fixed inset-0 z-50 overflow-y-auto p-4 md:p-8">
          ...
          <div className="relative z-10 mx-auto flex min-h-full w-full max-w-7xl items-center">
            <div className="border-brand-line bg-brand-panel/90 max-h-[calc(100vh-2rem)] w-full overflow-y-auto rounded-3xl border p-4 shadow-[0_26px_70px_rgba(0,0,0,0.55)] backdrop-blur-xl md:max-h-[calc(100vh-4rem)] md:p-6">
              <div className="mb-4 flex items-center justify-between gap-3">
                <div>
                  <p className="text-brand-secondary text-xs tracking-[0.2em] uppercase">
                    Tela de sorteio
                  </p>
                  <h2 className="font-heading text-brand-tertiary text-xl font-semibold md:text-2xl">
                    Sorteador oficial
                  </h2>
                </div>
                ...
                <button
                  type="button"
                  onClick={() => setIsDrawModalOpen(false)}
                  className="border-brand-line text-brand-tertiary hover:border-brand-secondary hover:text-brand-secondary rounded-xl border px-4 py-2 text-sm font-semibold transition"
                >
                  Fechar
                </button>
              </div>
